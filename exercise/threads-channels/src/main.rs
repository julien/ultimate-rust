...
fn main() {
...

    {
        ...
        if let Err(e) = handle_a.join() {
            println!("couldn't join handle_a: {:?}", e);
        }
        if let Err(e) = handle_b.join() {
            println!("couldn't join handle_b: {:?}", e);
        }
    }
    ...
    // the child threads.
    {
        ...
        let handle1 = thread::spawn(|| worker("handle1", rx2, tx4));
        let handle2 = thread::spawn(|| worker("handle2", rx, tx3));

        let values = vec![42, 666, 69];
        for value in values {
...
fn worker(name: &str, values: Receiver<i32>, result: Sender<i32>) {
    for value in values {
