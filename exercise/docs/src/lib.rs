...
//! often deep yellow to orange in coloration. The thick shell contains the seeds and pulp. The name is
//! most commonly used for cultivars of Cucurbita pepo, but some cultivars of Cucurbita maxima, C. argyrosperma, and C.
//! moschata with similar appearance are also sometimes called "pumpkins".
...
//! ![A bunch of pumpkins](https://upload.wikimedia.org/wikipedia/commons/thumb/5/5c/FrenchMarketPumpkinsB.jpg/700px-FrenchMarketPumpkinsB.jpg)
...
/// # Recipes
///
/// Recipes will be coming **soonish**.
pub struct Pumpkin {
    /// `roudness` is a percentage
    pub roundness: f32,
    /// `orangness` is number from 8 to 27
    pub orangeness: i32,
}
...
impl Pumpkin {
    /// The `smash` method destroys the pumpking which makes it unavailable for a pie
    pub fn smash(self) {}
}
