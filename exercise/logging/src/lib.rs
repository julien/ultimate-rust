...
impl Frog {
    pub fn new() -> Self {
        ...
        debug!(target: "Frog::new", "A new Frog has beem created");
        Default::default()
    }
    pub fn hop(&mut self) {
        self.energy -= 1;
        // 3. Use info!() to log that a Frog hopped, and how much energy is left
        info!(target: "Frog::hop", "The frogged hopped (energy level: {})", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog::hop", "The frog will go to sleep because it has no energy");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target: "Frog::sleep", "The frog is already alseep");
        } else {
            self.sleeping = true;
        }
    }
